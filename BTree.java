...
public class BTree<T extends Comparable<T>> {
	...
	// we add List of Lists because we dont know how much space of list we need
	protected LinkedList<LinkedList<T>> backtrack = new LinkedList<LinkedList<T>>();
	 ...
	 * of children in a non-root node.
	 *
	 * @param order the minimal degree of a node of the B-Tree.
	 */
	...
	public void insert(T value) {
		// we create list that contains the node we added and the splitted node
		LinkedList<T> splitNodes = new LinkedList<T>();
		splitNodes.addLast(value);
		if (root == null) {
			...
			while (currentNode != null && !wasAdded) {
				...
				if (currentNode.getNumberOfKeys() == maxDegree - 1) {
					...
					// we add the splitted node to the list
					splitNodes.addLast(currentNode.getKey((currentNode.getNumberOfKeys()) / 2));
					split(currentNode);
		...
		// we added the list we created to the backtrack
		backtrack.addFirst(splitNodes);
		size++;
	 ...
	 *
	 * @param nodeToSplit a node with (2 * order - 1) keys
	 */
	...
	protected static class Node<T extends Comparable<T>> {
		...
		public String toString() {
			...
			if (children != null) {
				builder.append("keySize=").append(getNumberOfKeys()).append(" children=").append(getNumberOfChildren())
						.append("\n");
			}
